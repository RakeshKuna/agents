...
import AgentBranchProfileCreate from './AgentBranchProfileCreate';
        ...
        .then((response)=>{
          console.log(response);
            if(response.status == 200){
              this.setState({loading:false},()=>{
                  this.setState({agentProfile:response.data,value:0},()=>{
                    let breadcrumpData = [{link:'/agentprofiles',text:'Agent Profiles'},{link:`/agentprofiles/${this.props.match.params.agentid}/branches`,text: response.data.name +' ('+ this.props.match.params.agentid +')'},{link:'#',text:'Create'}];
      ...
      return (
        <div>
          <Grid container spacing={24}>
            ...
            <Grid item xs={12}>              
                <p className="agent-branch-header">Agent Branch Profile (Create)</p>                          
            </Grid>
          </Grid>
                ...
                <div>
                  <AppBar position="static" className={classes.appbar}>
                    ...
                    <Tabs value={this.state.value} umStyle="primary" onChange={this.handleChange}>
                      <Tab className ="global-font" label="AGENT BRANCH PROFILE DETAILS" />
                      <Tab className ="global-font" label="ADDITIONAL AGENT BRANCH PROFILE DETAILS" isEnabled={false} />
                    </Tabs>
                  </AppBar>
                  {this.state.value === 0 && <AgentBranchProfileCreate handleCompValueChange={this.handleCompValueChange} agentProfile={this.state.agentProfile} {...this.props}/>}
