...
import ViewAllowedProducts from './ViewAllowedProducts'
...
class EnchancedTabsviewAllowedProducts extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      value: 0,
    ...
    if(sessionStorage.getItem('token') == undefined){
      ...
      .then((response) => {
        if (response.status == 200) {
          ...
          this.setState({ allowedProductsProfile:response.data,loading: false}, () => {
            this.getActivity();
  ...
  getActivity = () => {
    if(sessionStorage.getItem('token') == undefined){
      ...
      this.setState({ loading: true }, () => {
        ApiService.getActivity(this.props.match.params.agentid,this.props.match.params.agentbranchId,this.props.match.params.allowedproductsid,headers)
          .then((response) => {
            if (response.status == 200) {
              console.log(response);
                  ...
                  <div>
                    <AppBar position="static" className={classes.appbar}>
                      ...
                      <Tabs value={value} umStyle="primary" onChange={this.handleChange}>
                        <Tab style={{ fontSize: "16px", fontFamily: "Gotham-Rounded" }} label="ALLOWED PRODUCT SEND DETAILS" />
                        <Tab style={{ fontSize: "16px", fontFamily: "Gotham-Rounded" }} label="ACTIVITY" />
                      </Tabs>
                    </AppBar>
                    {this.state.value === 0 && <ViewAllowedProducts allowedProductsProfile={this.state.allowedProductsProfile}/>}
...
EnchancedTabsviewAllowedProducts.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(EnchancedTabsviewAllowedProducts);
